...
class MainPage(tk.Frame):
    def __init__(self, container, controller):
        ...
        button1 = BigButton(self, text="ADD EXPERIMENT", height=int(GUI_HEIGHT / 4),
                            width=int(GUI_WIDTH / 4),  # style= 'Big.TButton',
                            command=lambda: self.add_experiment())
        button1.grid(row=0, column=0)
        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(0, weight=1)

        button2 = BigButton(self, text="ADD DATASET", height=int(GUI_HEIGHT / 4),
                            width=int(GUI_WIDTH / 4),  # style= 'Big.TButton',
                            command=lambda: self.add_dataset())
